...
function initTree() {
    vm.visible.treeVisible = true;
    $.ajax({
        url: _path + "/region/regionJson",
        success: function (result) {
            if (result.code === '0') {
                initDynamicTree(JSON.parse(result.data));
            } else {
                vm.visible.treeVisible = false;
            }
        }
    });
}
...
function initDynamicTree(flare) {
    ...
    var svg = d3.select("#regionalAnalyze").append("svg")
